#!/bin/bash
# Упрощенная проверка статистики
...
docker-compose -f docker-compose.prod.yml exec -T mysql mysql -uroot -pRentSense2025\!Secure rentsense -e "SELECT COUNT(*) as total_offers FROM offers;" 2>/dev/null | tail -1
echo ""
...
if [ -n "$TWO_HOUR" ] && [ "$TWO_HOUR" != "count" ]; then
...
if [ -n "$ONE_HOUR" ] && [ "$ONE_HOUR" != "count" ]; then
