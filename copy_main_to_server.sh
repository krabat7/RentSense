#!/bin/bash
# Команда для копирования исправленного main.py на сервер
# Использование: скопируйте содержимое этого файла и выполните на сервере

cd /root/rentsense

cat > app/parser/main.py << 'MAIN_EOF'
import logging
import random
import time
import re
...
python3 -m py_compile app/parser/main.py && echo "✓ Синтаксис корректен" || (echo "✗ Ошибка синтаксиса" && exit 1)
