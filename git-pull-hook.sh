...
# Сбрасываем локальные изменения во всех файлах (включая сам скрипт)
# Это безопасно, так как скрипт запускается из cron и должен быть синхронизирован с репозиторием
